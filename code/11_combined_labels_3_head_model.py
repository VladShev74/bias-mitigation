...
def train_model(model_name: str, model_id: str, train_dataset, seed: int, save_path: Path):
    """
    Train a three-head model with specified seed.
    ...
    for epoch in range(NUM_EPOCHS):
        total_task_loss, total_gender_loss, total_age_loss = 0, 0, 0
        num_batches = 0
        ...
        for batch in progress_bar:
            ...
            optimizer.zero_grad()
            task_logits, gender_logits, age_logits = model(input_ids, attention_mask)
            ...
            total_loss = task_loss + gender_loss + age_loss
        ...
        print(f"  [Epoch {epoch+1}] Task: {avg_task_loss:.4f} | "
              f"Gender: {avg_gender_loss:.4f} | Age: {avg_age_loss:.4f}")
...
def evaluate_model(model, val_df, tokenizer):
    ...
    Returns:
        Tuple of (task_acc, gender_acc, gender_bal_acc, age_acc, age_bal_acc)
    """
    model.eval()
    val_dataset = CustomDatasetThreeHeads(val_df, tokenizer)
    ...
    age_tp = {i: 0 for i in range(5)}
    age_total = {i: 0 for i in range(5)}
    ...
    return task_accuracy, gender_accuracy, gender_balanced_accuracy, age_accuracy, age_balanced_accuracy


def train_and_evaluate_all_models():
    """Train and evaluate all models across all seeds."""
    overall_start = datetime.now()
    print(f"\n[{overall_start.strftime('%H:%M:%S')}] Starting three-head training pipeline...")
    ...
    for model_name, model_id in MODEL_IDS.items():
        ...
        train_dataset = CustomDatasetThreeHeads(train_df, tokenizer)

        model_results = []

        for seed in SEEDS:
            # Define save path: models/three_head_combined/{model_name}/seed_{seed}/
            save_path = PROJECT_ROOT / "models" / "three_head_combined" / model_name / f"seed_{seed}"
            ...
            results_seed_dir = PROJECT_ROOT / "results" / "three_head_training_combined" / model_name / f"seed_{seed}"
            ...
            if eval_file_models.exists() and eval_file_results.exists():
                ...
                task_acc, gender_acc, gender_bal_acc, age_acc, age_bal_acc = metrics
        ...
        model_results_dir = PROJECT_ROOT / "results" / "three_head_training_combined" / model_name
    ...
    results_dir = PROJECT_ROOT / "results" / "three_head_training_combined"
    ...
    print(f"\n{'='*70}")
    print("FINAL EVALUATION SUMMARY (THREE HEADS)")
    print(f"{'='*70}\n")
    ...
    for model_name, model_results in results.items():
        ...
        for seed_result in model_results['per_seed_results']:
            print(f"    Seed {seed_result['seed']:4d}: Task={seed_result['task_accuracy']:.4f} | "
                  f"Gender={seed_result['gender_accuracy']:.4f} ({seed_result['gender_balanced_accuracy']:.4f}) | "
                  f"Age={seed_result['age_accuracy']:.4f} ({seed_result['age_balanced_accuracy']:.4f})")
        print()
