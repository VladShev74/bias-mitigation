...
def load_intervention_map(model_name):
    """Load combined neuron intervention map for the model."""
    map_path = PROJECT_ROOT / "results" / "activation_differences" / model_name / "neuron_map_combined.json"
...
def load_baseline_results(model_name):
    """Load baseline performance from training results (averaged across seeds)."""
...
def evaluate_model(model, texts, tokenizer, task_labels, gender_labels, age_labels, model_name):
    ...
    return (float(task_accuracy), float(gender_accuracy), float(gender_balanced_accuracy),
            float(age_accuracy), float(age_balanced_accuracy))


def run_experiment(model, texts, tokenizer, task_labels, gender_labels, age_labels,
                   mode, scale, coverage, layers_strategy, intervention_map, model_name):
    """
    ...
    Args:
        ...
        intervention_map: Full neuron map (combined)
        model_name: 'bert' or 'modern_bert'
    ...
    # Evaluate
    task_acc, gender_acc, gender_bal_acc, age_acc, age_bal_acc = evaluate_model(
        model, texts, tokenizer, task_labels, gender_labels, age_labels, model_name
    )
...
def run_seed_experiments(model_name, seed, test_df, tokenizer, intervention_map):
    ...
    seed_dir = PROJECT_ROOT / "results" / "neuron_scaling_bias_mitigation_combined" / model_name / f"seed_{seed}"
    ...
    # Initialize model architecture (BertWithThreeHeads works for both BERT and Modern BERT)
...
def main():
    ...
    print(f"\n{'#'*70}")
    print("# Multi-seed Combined Bias Mitigation (3-Head Model)")
    ...
    for model_name, model_id in MODEL_IDS.items():
        ...
        # Load combined intervention map
        print("[OK] Loading combined intervention map...")
        ...
        model_dir = PROJECT_ROOT / "results" / "neuron_scaling_bias_mitigation_combined" / model_name
